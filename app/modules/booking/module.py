...
class BookingModule:
    ...
    def __init__(self, env:environment.Environment, global_config: base_config.Config):
        ...
        self.env = env
        booking_repo = repository.Repository(self.env.http.db)
        booking_service = service.Service(repo=booking_repo)
        booking_handler = handler.Handler(svc=booking_service, cfgs=self.config)
        self.booking_controller = controller.Controller(h=booking_handler, cfgs=self.config)
        self.register_route()


    def register_route(self):
        return routes.register_routes(self.env.http.get_app(), self.booking_controller)
